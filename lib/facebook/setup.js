    ...
    let fbHandle = parsedMessage => {
      if (parsedMessage) {
        ...
        return Promise.resolve(parsedMessage).then(parsedMessage => bot(parsedMessage, request))
          .then(botResponse => responder(recipient, botResponse, request.env.facebookAccessTokens[p_id]))
          .catch(logError);
      }
    };
    ...
    return Promise.resolve().then(() => {
      return utils.apiGatewayPromise.getStagePromise({
        ...
        if (options['configure-fb-bot']) {
          ...
          .then(results => {
            console.log('\n');
            let fbAuthTokens = require('./fbAuthTokens.json');
            console.log('fbAuthTokens', fbAuthTokens);
            ...
            let pageAccessTokens = JSON.parse(JSON.stringify(results[fbAuthTokenPrompt]))

            console.log('pageAccessTokens', pageAccessTokens)
            console.log('pageAccessTokens[0]', pageAccessTokens[0])
            ...
            const deployment = {
              ...
              variables: {
                ...
                facebookAccessTokens: pageAccessDic
              }
            };
